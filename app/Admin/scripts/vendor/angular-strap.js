...
angular.module('$strap.directives').factory('$modalStrap', [
		'$rootScope',
		'$compile',
		'$http',
		'$timeout',
			...
			var ModalFactory = function ModalFactory(config) {
				function Modal(config) {
					...
					return $q.when($templateCache.get(templateUrl) || $http.get(templateUrl, { cache: true }).then(function (res) {
							...
							var $modalStrap = $('<div class="modal hide" tabindex="-1"></div>').attr('id', id).addClass('fade').html(template);
							if (options.modalClass)
								$modalStrap.addClass(options.modalClass);
							$('body').append($modalStrap);
							$timeout(function () {
								$compile($modalStrap)(scope);
							});
							scope.$modalStrap = function (name) {
								$modalStrap.modal(name);
							};
							angular.forEach([
								'show',
								'hide'
							...
							angular.forEach([
								...
								$modalStrap.on(name, function (ev) {
									scope.$emit('modal-' + name, ev);
								});
							});
							$modalStrap.on('shown', function (ev) {
								$('input[autofocus], textarea[autofocus]', $modalStrap).first().trigger('focus');
							});
							$modalStrap.on('hidden', function (ev) {
								if (!options.persist)
									scope.$destroy();
							});
							scope.$on('$destroy', function () {
		...
		function ($q, $modalStrap) {
			return {
				...
				link: function postLink(scope, iElement, iAttrs, controller) {
					...
					$q.when($modalStrap(options)).then(function onSuccess(modal) {
